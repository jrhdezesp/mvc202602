CREATE TABLE `products` (
  ...
  -- Reactivar Ofertas (sales) por 30 días desde hoy
UPDATE sales
SET
  saleStart = NOW() - INTERVAL 1 DAY,
  saleEnd   = NOW() + INTERVAL 30 DAY
WHERE productId IN (3, 5, 7);

-- Reactivar Destacados (highlights) por 30 días desde hoy
UPDATE highlights
SET
  highlightStart = NOW() - INTERVAL 1 DAY,
  highlightEnd   = NOW() + INTERVAL 30 DAY
WHERE productId IN (1, 4);

-- Verificación rápida
SELECT 'sales vigentes' AS tipo, COUNT(*) AS total
FROM sales
WHERE saleStart <= NOW() AND saleEnd >= NOW()
UNION ALL
SELECT 'highlights vigentes' AS tipo, COUNT(*) AS total
FROM highlights
WHERE highlightStart <= NOW() AND highlightEnd >= NOW();